...
def make_twist_msg(accel, acc_omega, goalData, warn_w, yaw_meas, last_point):
    global vel_up
    global vel_down
    global w_up
    global latest_yaw
    ...
    delta_step = rospy.has_param('/delta_step') and rospy.get_param('/delta_step')
    if (not goalData[0] and not delta_step):
        ...
        rospy.logwarn("DELTA STEP IN NAV")
        cmd_w_ = w_up + dt_in*acc_omega
        w_up = cmd_w_ 
        dToGoal = goalData[1]
        cmd_vel_ = vel_down - dt_in*vel_down*vel_down/(5*dToGoal)

        # Reset counter if starting from standstill
        global new_delta
        global ctr
        if new_delta:
            new_delta = False
        ...
        # Send vel + acc
        cmd.linear.x = defs.DELTA_STEP_SPEED
        cmd.angular.z = w_up
...
def delete_pruning_points_from_file():
    ...
    if can_delete_file:
        try:
            path = "/home/amiga/catkin_workspaces/nimo_ws/src/MPC_Amiga/gps_coordinates/"
...
def mpc_node():
    ...
    while not rospy.is_shutdown():
        ...
        # print(f"pruning_sttaus: {rospy.get_param('/pruning_status')}")
        ...
        #    publish_marker(ppx, ppy)  
        #    last_time = current_time
        while not rospy.get_param("/pruning_status"): 
            rate.sleep()
        # print("OUT OF WHILE")
        #current_time = rospy.Time.now()
